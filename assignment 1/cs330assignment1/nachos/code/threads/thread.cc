...
Thread::Thread(char* threadName)
{
    ...
    children_pid_exit_code = new List();
	pid = ++g_pid;
    wakeup_parent = FALSE;
	if(g_pid == 1) //to handle to first process
	{
		ppid = 0;
		parent = NULL;
	}	
...
Thread::Finish ()
{
    ...
    {
        if(currentThread->wakeup_parent)
        {
            scheduler->ReadyToRun(currentThread->parent);
        }
    }
	Num_Tot_Threads--;
    if(Num_Tot_Threads == 0)
    {
        printf("\nNo thread remaining......\n");
        interrupt->Halt();
    }
...
void SetUpContext(int arg)
{
	...
	//One More Thing To add  here something related to MIPSIM RUN
	machine -> Run();
...
void 
Thread::SetRegister( int regnum , int val)
{
	userRegisters[regnum] = val;
}
//----------------------------------------------------------------------
