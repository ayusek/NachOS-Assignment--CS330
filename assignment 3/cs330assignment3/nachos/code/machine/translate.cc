...
Machine::ReadMem(int addr, int size, int *value)
{
	...
	*value = WordToHost(data);
    ...
    DEBUG('r',"Just accessed the page %d\n",physicalAddress/PageSize);
    physPageLRU[physicalAddress/PageSize] = stats->totalTicks;
    physPageLRUclock[physicalAddress/PageSize] = 1;
    return (TRUE);
}
...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
	    ...
	    DEBUG('r', "virtual page # %d not valid %d!\n", 
			virtAddr, pageTableSize);
