...
int priorityValue[MAX_THREAD_COUNT];    //Array to maintain priority value of threads
int cpuCount[MAX_THREAD_COUNT];
ThreadStatus threadStatusByPid[MAX_THREAD_COUNT];
int totalWait = 0;
//*************************************************
...
{
    //printf("Current timer %d thread %d  status %d\n" , stats -> totalTicks , currentThread -> pid , currentThread -> getStatus());
    ...
    if (interrupt->getStatus() != IdleMode) {
        ...
        {   
            if( (stats->totalTicks - stats->start_tick_current_burst) >= timer->TimeOfNextInterrupt() )
